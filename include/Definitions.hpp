...
//------Tapered Evaluation-------
    struct Score {
        ...
        Score& operator -=(const Score& other) { mg -= other.mg; eg -= other.eg; return *this; }
        Score operator -() const { return {-mg, -eg}; } // Negation for Black pieces
    };

    const Score PieceValues[6] = {
        {82, 94},     // PAWN
        {337, 281},   // KNIGHT
        {365, 297},   // BISHOP
        {477, 512},   // ROOK
    ...
    // --- Phase values for each piece (same order as Piece Enum) ---
    const int gamephaseInc[13] = {0, 0, 1, 1, 2, 4, 0, 0, 1, 1, 2, 4, 0}; 

    // --- Positional Tables (Mapped A1 to H8) ---
    const Score PestoPawn[64] = {
    ...
    //Array of arrays:
    const Score* const PestoTables[6] = { PestoPawn, PestoKnight, PestoBishop, PestoRook, PestoQueen, PestoKing };